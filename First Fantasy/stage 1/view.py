...
TERRAIN_COLORS = {
    model.TERRAIN_GROUND: (69, 39, 2),
    model.TERRAIN_RADIATED_GROUND: (40, 240, 5),
    model.TERRAIN_BLOCKED: (192, 0, 0)
}
...
class MapView(pg.sprite.Sprite):
    ...
    def render(self,
        ...
        self.image.blit(self.images[model.PLAYER], player.rect)
...
class MiniMapView(pg.sprite.Sprite):
    ...
    def render(self,
        ...
        translated_rect.y *= scale_factor
        translated_rect.move_ip(cen_x, cen_y)
        self.image.blit(self.images[model.PLAYER], translated_rect)
...
class Renderer:
    ...
    def create_assets(self) -> None:
        ...
        self.images = {
            model.TERRAIN_GROUND: pg.Surface((32,32)),
            model.TERRAIN_RADIATED_GROUND: pg.Surface((32,32)),
            model.TERRAIN_BLOCKED: pg.Surface((32,32)),
            model.PLAYER: pg.Surface((24,24))
        }

        self.images[model.TERRAIN_GROUND].fill(TERRAIN_COLORS[model.TERRAIN_GROUND])
        self.images[model.TERRAIN_RADIATED_GROUND].fill(
            TERRAIN_COLORS[model.TERRAIN_RADIATED_GROUND]
        )
        self.images[model.TERRAIN_BLOCKED].fill(TERRAIN_COLORS[model.TERRAIN_BLOCKED])
        self.images[model.PLAYER].fill(PLAYER_COLOR)
    ...
    def draw(self,
        ...
        self.map_view.render(terrain,player)
        self.mini_map_view.render(terrain,player)
